...
{	
  ...
  if(edep>threshold || abs(deltae)>threshold){    // Cut energy deposit > 1keV
    ...
    G4double x0=step->GetPreStepPoint()->GetPosition().x();
    G4double y0=step->GetPreStepPoint()->GetPosition().y();
    G4double z0=step->GetPreStepPoint()->GetPosition().z();
    ...
    // int min = 950;
    // int max = 999;
    // if (picells_in_cublet_idx<max && picells_in_cublet_idx>min) {
    
    //   G4cout << "cell in cublet index: " << picells_in_cublet_idx << G4endl;
    //   G4cout<< "x:" <<x0 << " y:" << y0 << " z:" << z0 << G4endl;
    //   G4cout << " " <<  G4endl;
    // }

    // if (picublet_idx<max && picublet_idx>min) {
    //   G4cout << "cublet index: " << picublet_idx << G4endl;
    //   G4cout<< "x:" <<x0 << " y:" << y0 << " z:" << z0 << G4endl;
    //   G4cout << " " <<  G4endl;
    // }
    ...
    // // Corrected calculations for a 100 x 100 x 200 grid
    // G4int z_idx = picell_idx / 10000;  // Now considering nCellsZ = 200
    // G4int y_idx = (picell_idx % 10000) / 100; 
    // G4int x_idx = (picell_idx % 100); 

    // G4double picublet_idx = z_idx * 10000 + y_idx * 100 + x_idx;

    // // Correcting cublet indices for 100 x 100 x 200
    // G4int z_idx_cub = (picell_idx / 1000) % 10;
    // G4int y_idx_cub = (picell_idx / 100) % 10;
    // G4int x_idx_cub = picell_idx % 10;
