...
// To compile: g++ -Ofast -g Ntuple_8.C Edep.C part_info.C `root-config --cflags` -o Ntuple_8 -L$(root-config --libdir)
// -Wl,-rpath,$(root-config --libdir) -lCore -lRIO -lNet -lHist -lGraf -lGraf3d -lGpad -lTree -lRint -lPostscript
// -lMatrix -lPhysics -lMathCore -lThread -lMultiProc -lROOTVecOps -pthread -lm -ldl Example: hadron.root for 5 events
// ->  secs to finish (+ map time)
/////////////////////////////////////////////////////////////////
#include <TCanvas.h>
#include <TChain.h>
#include <TFile.h>
...
void processRootFile(const char *filename, TTree *outputTree, const int max_event, int map_mode, int verbose)
{
    ...
    outputTree->Branch("Tinteractions_in_event", &Tentries);
    ...
    do
    {
        ...
        outputTree->Fill();
        if (verbose > 0)
            std::cout << "Event: " << ievent << " filled correctly!" << std::endl;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template <typename T> std::map<int, vector<UInt_t>> map_indices(T &event, int entries, int mode, std::string filename)
{
